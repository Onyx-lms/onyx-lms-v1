...
export default async function OnyxContestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    >
      <Link href="/onyx/contests" className="text-sm text-slate-600 hover:underline">
        &larr; All contests
      </Link>
      ...
      <div className="mt-4 grid gap-8 lg:grid-cols-[1fr_320px]">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-xl border border-slate-200 p-4 text-sm">
            <div className="text-xs uppercase tracking-wide text-slate-500">Problems</div>
            <ul className="mt-2 space-y-1">
              {(contest.problems ?? []).map((p) => (
                <li key={p.problem_id} className="flex justify-between gap-2">
                  ...
                  <span className="tabular-nums text-slate-600">{p.points}</span>
                </li>
              ))}
            </ul>
