...
export default async function OnyxInterviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    >
      <Link href="/onyx/interviews" className="text-sm text-slate-600 hover:underline">
        &larr; All interviews
      </Link>
          ...
          <section>
            ...
            <div className="mt-3 rounded-xl border border-slate-200 p-4">
              <div className="text-2xl font-semibold tabular-nums">
                {interview.overall} <span className="text-base text-slate-500">/ 5</span>
              </div>
                ...
                {interview.feedback.map((f, i) => (
                  <li key={i}>
                    <div className="flex justify-between gap-3">
                      ...
                      <span className="tabular-nums text-slate-600">{f.score} / {f.of}</span>
                    </div>
                    {f.comment ? (
